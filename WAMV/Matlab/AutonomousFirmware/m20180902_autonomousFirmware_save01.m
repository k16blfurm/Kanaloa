...
% Check to see if the thrusters are being published correctly. 
% Check to see if the rospublishers are publishing really quickly.
function m20180902_autonomousFirmware_save01()
...
function sub = createRosSubscibers()
    ...
    try
        ...
        try
        sub.wamvImu = rossubscriber('/clock',@clockCallback);
    catch
        disp(' ');
        disp('Cant communicate with clock!  Code will throw error!');
    end
...
function displayStuff(goal,pose,thrustCommands,error)
    ...
    %Check to see how long it takes to publish to each thruster
    fprintf('time [nsecs]: %12.0f\n',time);
    fprintf('Thrusters [-1 to 1]:  %3.2f, %3.2f ,\n\n',...
        Q1msg.Data,Q2msg.Data);
    %
...
function clockCallback(~,message)
