...
% For Open Source Robotics Foundation RobotX Simulation
% Changes to do:
% 1)Create Function to switch between Simulation mode and WAM-V mode
% 2) Create an updatePoseInSimulation function; which changes variable 
%    pose.tz from imutz to imutx.
% 3) Change control loop in autonomousFirmware function to switch between 
%    updatePose and updatePoseInSimulation function depending on mode.
%    Recomend if loop.
function m20180906_autonomousFirmware_save00()
...
function pub = createRosPublishers()
    ...
    pub.autoQ1 = rospublisher('/right_thrust_cmd','std_msgs/Float32');
    pub.autoQ2 = rospublisher('/left_thrust_cmd','std_msgs/Float32');
    %pub.autoQ3 = rospublisher('/left_rear_thrust_cmd','std_msgs/Float32');
    %pub.autoQ4 = rospublisher('/right_rear_thrust_cmd','std_msgs/Float32');
...
function sub = createRosSubscibers()
    ...
    try
        sub.wamvGps = rossubscriber('/gps/fix',@wamvGpsCallback);
    catch
        disp(' ');
    ...
    try
        sub.wamvImu = rossubscriber('/imu/data',@imuCallback);
    catch
        disp(' ');
    ...
    try
        ...
        sub.tzGoalInput = rossubscriber('/tzGoalInput',@tzGoalCallback);
    catch
        disp(' ');
...
%        error.xDist2goal))));
    ...
    % Map output vector, u, to -1 to 1 (what the topic wants)
    thrustCommands.Q1 = thrustCommands.Q1*(2/110);
    thrustCommands.Q2 = thrustCommands.Q2*(2/110);
    %thrustCommands.Q3 = round(thrustCommands.Q3*(2/110),0);
    %thrustCommands.Q4 = round(thrustCommands.Q4*(2/110),0);
...
function displayStuff(goal,pose,thrustCommands,error)
    ...
    fprintf('Euler Angle X,Y,Z [Deg]: %3.2f %3.2f %3.2f\n', rad2deg(imutx),rad2deg(imuty),rad2deg(imutz));
    fprintf('Heading error [deg]: %.1f\n',rad2deg(error.head2goal));
    fprintf('Path heading error [deg]: %.1f\n\n',rad2deg(error.pathHead2goal));
    fprintf('Thrusters [-1 to 1]:  %3.2f, %3.2f,\n\n',...
