...
void loop() {
  ...
  // Check mode (manual or auto) from transmitter
    ...
    //Use these lines to look at voltage
    /*
  ...
  if (modeRead<=manualPWM) {  // if WAMV is in manual mode
    ...
    //If you do not know how pointers work, I apologize for this system.
    int Q1Map=0;
    int Q2Map=0;
    int Q3Map=0;
    int Q4Map=0;
...
float batteryRead(){
  ...
  float mainVolt = voltMainBit *(45.0/1023.0);         // voltage on voltMainPin [V] Was 21.0/1024.0
...
void transmittermap(int *Q1,int *Q2,int *Q3,int *Q4){
  ...
  // Read in receiver values
    int ch1 = pulseIn(ch1Pin, HIGH);   // PWM in from remote control receiver channel 1 (right stick left-to-right yaw/rotation) Full Left: 1075 Full Right: 1900
    int ch6 = pulseIn(ch6Pin, HIGH);   // PWM in from remote control receiver channel 6 (left stick up-to-down surge/forward) Same range
    int ch4 = pulseIn(ch4Pin, HIGH);   // PWM in from remote control receiver channel 4 (left stick left-to-right sway/strafe) Full Left: 1075 Full Right: 1900
