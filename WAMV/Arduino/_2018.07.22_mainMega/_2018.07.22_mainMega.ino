...
//Note: Need to switch back modePin and 
...
void loop() {
  ...
  // Check mode (manual or auto) from transmitter
    modeRead= pulseIn(modeRead,HIGH); //1081;  //pulseIn(modePin,HIGH);

  // ====================
  //     MANUAL CODE  
  // ====================

  if (modeRead>=manualPWM) {  // if switch is 
...
float batteryRead(){
  ...
  float mainVolt = voltMainBit*(21.0/1024.0);      // voltage on voltMainPin [V]
