...
void autoQ1Cb(const std_msgs::Float64& autoQ1CbMsg) {
  autoQ1 = map(autoQ1CbMsg.data, -120, 120, 31, 63);
}
void autoQ2Cb(const std_msgs::Float64& autoQ2CbMsg) {
  autoQ2 = map(autoQ2CbMsg.data, -120, 120, 31, 63);
}
void autoQ3Cb(const std_msgs::Float64& autoQ3CbMsg) {
  autoQ3 = map(autoQ3CbMsg.data, -120, 120, 31, 63);
}
void autoQ4Cb(const std_msgs::Float64& autoQ4CbMsg) {
  autoQ4 = map(autoQ4CbMsg.data, -120, 120, 31, 63);
}
...
void loop() {
  ...
  int ch8 = pulseIn(ch8Pin, HIGH, 32000);

  // Kill as necessary
  if (ch8 > 1995*0.9 && ch8 < 1995*1.1) {       // ch8 (kill switch) pulse length approximately 2045 when when switch G is UP
    digitalWrite(thrusterKillPin, HIGH);
    digitalWrite(armReelKillPin, HIGH); 
  }
  else if (ch8 > 1491*0.9 && ch8 < 1497*1.1) {   // ch8 (kill switch) pulse length approximately 1491 to 1497 when when switch G is MID
    digitalWrite(thrusterKillPin, HIGH);
  ...
  float voltMain = voltMainBit*(21.0/1024.0);      // voltage on voltMainPin [V]
  ...
  int ch5 = pulseIn(ch5Pin, HIGH, 32000);

  // ====================
  //     MANUAL CODE  
  // ====================

  if (ch5 > 970*0.9 && ch5 < 995*1.1) {  // ch5 (manual or auto switch) pulse length approximately 925 to 943 when when switch A is UP
    ...
    int ch1Map = map(ch1, 981, 1999, -100, 100);   // yaw: 981 = full left. 1999 = full right. 
    int ch3Map = map(ch3, 981, 1999, -100, 100);   // surge: 981 = full down. 1999 = full up.
    int ch4Map = map(ch4, 978, 1996, 100, -100);   // sway: 978 = full left. 1996 = full right.
    ...
    int Q1Out = map(Q1Map, -120, 120, 31, 63);
    int Q2Out = map(Q2Map, -120, 120, 31, 63);
    int Q3Out = map(Q3Map, -120, 120, 31, 63);
    int Q4Out = map(Q4Map, -120, 120, 31, 63);
  ...
  else if (ch5 > 2035*0.9 && ch5 < 2053*1.1) {  // ch5 (manual or auto switch) pulse length approximately 2035 to 2053 when when switch A is UP

    // Set safety light
    digitalWrite(yellowLightPin,LOW);     // turn off yellow safety light
    digitalWrite(greenLightPin,HIGH);     // turn on green safety light

    // Adjust motor drive output based on main battery voltage (at this point, autoQn is already mapped)
    int Q1Out = 47+(autoQ1-47)*12/voltMain;
    int Q2Out = 47+(autoQ2-47)*12/voltMain;
    int Q3Out = 47+(autoQ3-47)*12/voltMain;
    int Q4Out = 47+(autoQ4-47)*12/voltMain; 
  ...
  else {    // noise on the ch5 (manual or autonomous switch)
